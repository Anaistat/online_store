...
const ProductCard:FC<CardProps> = observer(({
    product,
    isOutOfStock
}) => {

    const articleClass = [style['product-card']]
    if(isOutOfStock){
        articleClass.push(style['product-card--out'])
    }
    ...
    return (
        <article className={articleClass.join(' ')}>
            <Link to='/product/1' className={style['product-link']}>
                <img src={product.images[0]} width='354' height='330' alt="product"/>
            </Link>
            <div className={style['product-info']}>
                <p className={style['product-info__name']}>{product.name}</p>
                <p className={style['product-info__price']}>${product.price}</p>
            </div>
            {
                isOutOfStock
                    ?
                    <div className={style['out-of-stock']}>
                        <p className={style['out-of-stock__text']}>OUT OF STOCK</p>
                    </div>
                    :
                    <></>
            }
